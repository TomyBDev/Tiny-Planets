...
void AARPlaneActor::BeginPlay()
{
	...
	// Create the dynamic material instance with a random color when a new plane is created
...
void AARPlaneActor::Tick(float DeltaTime)
{
	...
	// Match the planes actors location with the ARCore planes actor.
	PlanePolygonMeshComponent->SetWorldTransform(ARCorePlaneObject->GetLocalToWorldTransform());

	// Set the visibility of the plane based on the tracking state of the ARCore plane.
